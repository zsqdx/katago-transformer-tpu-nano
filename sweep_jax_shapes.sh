...
SWEEP_SPECS="${SWEEP_SPECS:-b24c1024:16 b12c1536:16 b12c1536:32 b16c1536:16 b12c1792:16 b8c2048:16 b8c2048:32 b10c2048:16 b12c2048:16 b14c2048:8 b16c2048:8 b16c2048:16 b18c2048:8 b20c2048:8 b22c2048:8}"
...
SWEEP_FAST_BF16="${SWEEP_FAST_BF16:-0}"

if [ "${SWEEP_FAST_BF16}" != "0" ]; then
...
echo "  fast_bf16: ${SWEEP_FAST_BF16}"
...
for spec in ${SWEEP_SPECS}; do
    ...
    if [ "${SWEEP_TEE}" != "0" ]; then
        (
            ...
            ROPE_DTYPE="${ROPE_DTYPE:-float32}" \
            FFN_MUL_DTYPE="${FFN_MUL_DTYPE:-float32}" \
            ATTENTION_LOGITS_DTYPE="${ATTENTION_LOGITS_DTYPE:-float32}" \
            ...
            DONATE_TRAIN_BUFFERS="${DONATE_TRAIN_BUFFERS:-0}" \
            NO_RESUME=1 \
            NO_FINAL_SAVE=1 \
            TRAINDIR="${traindir}" \
            bash train_jax.sh
